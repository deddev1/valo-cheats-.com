# Deploy valocheats.com

Step-by-step guide to deploy the Valorant Cheats static site to **valocheats.com** on Cloudflare Pages, configure DNS, and submit to Google Search Console.

## Prerequisites

- Node.js **≥ 22.12.0**
- Cloudflare account with access to **valocheats.com** DNS
...
This runs `wrangler pages deploy dist --project-name=bestvalorantcheats` (see `wrangler.toml`).

## 3. Custom domain and DNS

Add **valocheats.com** as the primary custom domain on the Pages project.
...
2. In **Rules** → **Redirect Rules** (or Bulk Redirects), create:
   - **Source:** `www.valocheats.com/*`
   - **Target:** `https://valocheats.com/${1}`
   - **Status:** 301

The deployed `functions/_middleware.js` also enforces apex canonical host, legacy domain redirects (`valocheats.com`, `.net`, `.com`), and legacy path redirects.
...
- `https://valocheats.com/es/`
- `https://valocheats.com/valorant-cheats/`
- `https://valocheats.com/valorant-aimbot/`
- `https://valocheats.com/sitemap.xml`
- `https://valocheats.com/robots.txt`

Verify redirects:

- `http://valocheats.com` → `https://valocheats.com` (301)
- `https://www.valocheats.com` → `https://valocheats.com` (301)
- Legacy domains (e.g. `valocheats.com`) → `https://valocheats.com` (301)
...
2. **Add property** → choose **Domain** → enter `valocheats.com`.
...
4. After verification, open **Sitemaps** and submit:
   ...
   https://valocheats.com/sitemap.xml
   ```
   Remove any legacy submissions (`sitemap-index.xml`, old `valocheats.com` URLs).
5. Use **URL Inspection** to request indexing for:
   - Homepage (`/`)
   - Pillar page (`/valorant-cheats/`)
   - Key landing pages (`/valorant-aimbot/`, `/valorant-esp/`, `/valorant-cheats-2026/`, etc.)
...
- [ ] Custom domain `valocheats.com` attached and active
- [ ] `www` redirects to apex
- [ ] Legacy domains 301 to `valocheats.com`
- [ ] Always Use HTTPS enabled
- [ ] `robots.txt` and sitemaps serve from `https://valocheats.com`
- [ ] Google Search Console domain verified
- [ ] `sitemap.xml` submitted in GSC
- [ ] Homepage and `/valorant-cheats/` requested for indexing
